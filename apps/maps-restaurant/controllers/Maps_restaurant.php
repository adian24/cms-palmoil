...
*
***/
class Maps_restaurant extends SITE_Controller
{
    private $page_curr = "";
    function __construct(){
        parent::__construct();
        $this->fragment['site']['module'] = "maps-restaurant";
        $this->page_curr = "maps-restaurant";
        $this->load->model("mapsres_model", "mod");
    }
    function index()
    {
        ...
        $this->fragment['site']['js'] = [
            ...
            base_url("assets/js/maps/restaurant").",".ENVIRONMENT.",1.3",
            base_url("assets/js/default").",".ENVIRONMENT.",1.3",
        ];
        $this->fragment['data']         = $this->mod->render();
        $this->fragment['page_nav']     = "maps::maps-restaurant";
        $this->fragment['page_title']   = "Maps - Restaurant";
    ...
    function forms()
    {
        ...
        $this->fragment['site']['js'] = [
            base_url("assets/plugins/datatables/jquery.dataTables").",min,1.0.0",
            base_url("assets/plugins/datatables/dataTables.bootstrap").",min,1.0.0",
            base_url("assets/js/maps/res-form").",".ENVIRONMENT.",1.1",
            base_url("assets/js/view_image").",".ENVIRONMENT.",1.1",
        ];
        $this->fragment['data_iso']     = $this->mod->render_iso(1);
        $this->fragment['page_nav']     = "maps::maps-restaurant";
        $this->fragment['page_title']   = "Maps Restaurant - Forms";
        $this->fragment['page_curr']    = $this->page_curr;
    ...
    function forms_save()
    {
        ...
        $json_loc = "./../".API_FOLDER."/jscache/cmap_1.json";
        $data_save = $this->mod->render_map(1);
        if ( $data_save )
        {
            foreach($data_save as $t)
            {
                $temp_photo = ASSET_URL."restaurant/".$t->photo;
                $t->photo = $temp_photo;
        ...
        if ($isnew == TRUE && empty($cek_map_name)){
            ...
            $this->response['response'] = "<b>Nama Restaurant</b> sudah ada, Harap masukan <b>Nama</b> yang berbeda!.";
            $this->response['uri']      = base_url($this->page_curr);
            echo json_encode($this->response);
            exit;
        }
