...
*
***/
class Maps_spbu extends SITE_Controller
{
    private $page_curr = "";
    function __construct(){
        parent::__construct();
        $this->fragment['site']['module'] = "maps-spbu";
        $this->page_curr = "maps-spbu";
        $this->load->model("mapsspbu_model", "mod");
    }
    function index()
    {
        ...
        $this->fragment['site']['js'] = [
            ...
            base_url("assets/js/maps/spbu").",".ENVIRONMENT.",1.3",
        ];
        $this->fragment['data']         = $this->mod->render();
        $this->fragment['page_nav']     = "maps::maps-spbu";
        $this->fragment['page_title']   = "Maps - SPBU";
    ...
    function forms()
    {
        ...
        $this->fragment['site']['js'] = [
            base_url("assets/plugins/datatables/jquery.dataTables").",min,1.0.0",
            base_url("assets/plugins/datatables/dataTables.bootstrap").",min,1.0.0",
            base_url("assets/js/maps/spbu-form").",".ENVIRONMENT.",1.1",
            base_url("assets/js/view_image").",".ENVIRONMENT.",1.1",
        ];
        $this->fragment['data_iso']     = $this->mod->render_iso(3);
        $this->fragment['page_nav']     = "maps::maps-spbu";
        $this->fragment['page_title']   = "Maps SPBU - Forms";
        $this->fragment['page_curr']    = $this->page_curr;
    ...
    function forms_save()
    {
        ...
        $json_loc = "./../".API_FOLDER."/jscache/cmap_3.json";
        $data_save = $this->mod->render_map(3);
        if ( $data_save )
        {
            foreach($data_save as $t)
            {
                $temp_photo = ASSET_URL."spbu/".$t->photo;
                $t->photo = $temp_photo;
        ...
        if ($isnew == TRUE && empty($cek_map_name)){
            ...
            $this->response['response'] = "<b>Nama SPBU</b> sudah ada, Harap masukan <b>Nama</b> yang berbeda!.";
            $this->response['uri']      = base_url($this->page_curr);
            echo json_encode($this->response);
            exit;
        }
